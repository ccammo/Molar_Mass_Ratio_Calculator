...
amount_pat = re.compile(r'(\d*.\d+)g')  # compound mass must end in the letter g
...
mass = float(amount_pat.search(compound)[1])
...
# Calculates and prints total Molar Mass for the compund
molar_mass = sum(el[1] * atomic_masses[el[0]] for el in elements)
print(f'\nTotal Molar Mass: {molar_mass}\n')

# Calculates and prints the mass for each element
for tup in elements:
    unit_mass = (mass / molar_mass) * atomic_masses[tup[0]] * tup[1]
    if unit_mass > 1000:
    	unit_mass /= 1000
    	unit = 'kg'
